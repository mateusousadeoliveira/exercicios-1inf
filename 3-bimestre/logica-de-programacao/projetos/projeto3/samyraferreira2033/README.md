# 🚀 PROJETO 3 — CREDENCIAMENTO DO CONGRESSO MÉDICO

# 🎯 PROBLEMA

Você faz parte da equipe de desenvolvimento que presta serviços de tecnologia para o Convenção Médica CardioSaúde.
...
- nome;
- idade;
- cargo (`participante` ou `palestrante` ou `organizador`);
...
Quando o(a) cargo for `palestrante` **ou** `organizador`, o resultado deverá ser:
...
```
Idade: 34 | Possui inscrição: true | Está bloqueado: false
...
```
Check-in do congresso confirmado
```

Em qualquer outro caso, o resultado deverá ser:

```
Check-in do congresso não confirmado
```

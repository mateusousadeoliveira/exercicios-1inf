# 🚀 PROJETO 3 — LIBERAÇÃO DA SALA DE GRAVAÇÃO

# 🎯 PROBLEMA

Você faz parte da equipe de desenvolvimento que presta serviços de tecnologia para o Estúdio de Gravação SomAlto.

Durante a reunião de **Sprint Planning**, a responsável pelo local explicou:

> "Antes de liberar a entrada de um(a) cliente, o sistema precisa verificar várias regras ao mesmo tempo: a idade mínima, se a pessoa possui reserva, se não está inadimplente e se o pagamento foi realizado. Só depois de checar tudo isso o sistema deve confirmar (ou não) o acesso."
>
...
- nome;
- idade;
- categoria (`comum` ou `produtor` ou `técnico`);
...
```
Nome: Gabriele Xavier
Idade: 27
Categoria: comum
Possui reserva: true
Está inadimplente: false
Valor da reserva: R$ 300
Valor pago: R$ 300
```
...
Quando o(a) categoria for `produtor` **ou** `técnico`, o resultado deverá ser:
...
- o(a) cliente possui reserva;
- o(a) cliente **não** está inadimplente.
...
```
Idade: 27 | Possui reserva: true | Está inadimplente: false

Resultado esperado: Acesso à sala liberado
```
...
```
Sessão de gravação confirmada
```

Em qualquer outro caso, o resultado deverá ser:

```
Sessão de gravação não confirmada
```

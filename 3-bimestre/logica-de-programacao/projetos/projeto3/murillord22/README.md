# 🚀 PROJETO 3 — LIBERAÇÃO DA MESA DE JOGOS

# 🎯 PROBLEMA

Você faz parte da equipe de desenvolvimento que presta serviços de tecnologia para o Sala de Jogos Board Game Café.

Durante a reunião de **Sprint Planning**, a responsável pelo local explicou:

> "Antes de liberar a entrada de um(a) cliente, o sistema precisa verificar várias regras ao mesmo tempo: a idade mínima, se a pessoa possui reserva, se não está bloqueado e se o pagamento foi realizado. Só depois de checar tudo isso o sistema deve confirmar (ou não) o acesso."
>
...
- nome;
- idade;
- categoria (`comum` ou `atendente` ou `coordenador`);
...
Quando o(a) categoria for `atendente` **ou** `coordenador`, o resultado deverá ser:
...
```
Acesso à mesa liberado
```

Caso contrário:

```
Acesso à mesa negado
```

Para o cenário principal:

```
Idade: 20 | Possui reserva: true | Está bloqueado: false

Resultado esperado: Acesso à mesa liberado
```
